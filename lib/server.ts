...
import { type CoapServerOptions, type requestListener, type CoapPacket, type Block, type MiddlewareParameters, type AddressInfo, type Parameters } from '../models/models'
...
class CoAPServer extends EventEmitter {
    ...
    _sendError (payload: Buffer, rsinfo: AddressInfo, packet?: CoapPacket, code = '5.00'): void {
        ...
        if (this._sock != null && typeof (this._sock as CoapSocket).send === 'function') {
            (this._sock as CoapSocket).send(message, 0, message.length, rsinfo.port, rsinfo.address)
        }
    }
