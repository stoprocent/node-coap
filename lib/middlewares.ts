...
import { randomBytes, timingSafeEqual } from './platform'
        ...
        .catch((err) => {
            if (err?.status === 201) {
                ...
                if (
                    ...
                    timingSafeEqual(innerEcho, storedNonce)
                ) {
                ...
                oscore.encode(innerResponse)
                    .then((encrypted) => {
                        const sock = request.server._sock
                        if (sock != null && typeof (sock as CoapSocket).send === 'function') {
                            (sock as CoapSocket).send(
